...
def main():
    ...
    n = 20
    iris_known_data = np.array([ np.random.choice(np.where(iris_labels==0)[0], n), [], np.random.choice(np.where(iris_labels==2)[0], n) ], dtype=object)
...
def print_title(title):
    ...
    spaces = int((((width-3)-len(title))/2)-1)
